...
/** Plain properties config: config/wnl_jeiboost.properties (written with defaults on first run). */
public final class Cfg {
    ...
    static void load() {
        Path p = Path.of("config", "wnl_jeiboost.properties");
            ...
            if (Files.isRegularFile(p)) {
                ...
                try (Writer w = Files.newBufferedWriter(p)) {
                    props.store(w, "JEIBoost — parallel JEI plugin registration + join-time caches.\n"
                            ...
                            + "creativeTabCache: cache the 'Registering ingredients' creative-tab scan to disk (local/wnl_jeiboost/creative-items.nbt) — saves ~10s per world join after the first. Auto-rebuilds when the mod set or relevant datapack registries change. Disabled automatically when JEI 'show creative tab names' or the creative-tab search mode are enabled (those features need the live scan).\n"
