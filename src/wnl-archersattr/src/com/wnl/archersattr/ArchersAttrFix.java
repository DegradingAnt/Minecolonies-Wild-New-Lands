...
public class ArchersAttrFix {
    public static final Logger LOGGER = LogManager.getLogger("wnl_archersattr");

    public ArchersAttrFix(IEventBus modBus) {
        modBus.addListener(EntityAttributeCreationEvent.class, this::onAttributes);
        LOGGER.info("[wnl_archersattr] loaded — will register missing attributes for archers_expansion summons");
    }
    ...
    private void onAttributes(EntityAttributeCreationEvent event) {
        ...
        if (n > 0) LOGGER.info("[wnl_archersattr] registered attributes for {} archers_expansion summon(s)", n);
    }
    ...
    private int register(EntityAttributeCreationEvent event, String className, String builderMethod) {
        try {
            ...
            LOGGER.warn("[wnl_archersattr] could not register attributes for {} ({}): {}",
