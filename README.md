# WNL — Custom Pack Work

Backup + source for the **custom** parts of the *WNL (Distant Horizons)* NeoForge 1.21.1 modpack.
...
- **`mods/`** — built custom add-on jars (drop-in for the instance's `mods/`):
  - `WNL-PackFixes` — runtime bytecode-fix coremod (Nashorn ASM), ~43 class-load-time guards so original mod jars stay untouched.
  - `WNL-DHSmooth` — client mixin that smooths Distant Horizons LOD draw-batch spikes (instant-return for re-shown terrain; adaptive ramp for new).
  - `WNL-MineColoniesCache` — MineColonies discovery-cache speedup + the "World mismatch" build-preview crash fix (client mixins).
  - `WNL-JEIBoost`, `-ArchersAttrFix`, `-FTBChunksOffload` — smaller targeted add-ons.
...
- **`datapacks/WNL-Compat/`** — the consolidated Paxi compat datapack (loot/recipe/tag/worldgen overrides + ore dupe-suppression + the entity loot-table fixes).
